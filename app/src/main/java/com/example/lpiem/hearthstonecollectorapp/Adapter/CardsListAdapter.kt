...
class CardsListAdapter(val items: List<Card>, val context: Context) : androidx.recyclerview.widget.RecyclerView.Adapter<ViewHolder>() {
    ...
    override fun onCreateViewHolder(p0: ViewGroup, p1: Int): ViewHolder {
        ...
        fragContext = context
        return ViewHolder(LayoutInflater.from(context).inflate(R.layout.cards_list_item, p0, false))
    }


    //Binds each card in the List to some views - p0=viewHolder, p1=position
    override fun onBindViewHolder(p0: ViewHolder, p1: Int) {
        p0?.updateWithUrl(items.get(p1).imgGold)
